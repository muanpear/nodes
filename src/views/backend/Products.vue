...
export default {
  ...
  methods: {
    ...
    //สร้างฟังก์ชั่นล้างข้อมูลเพิ่มสินค้า
    onResetForm() {
    ...
    // สร้างฟังก์ชั่น submit form สินค้า
    submitForm() {
      this.v$.$validate();
      if (!this.v$.$error) {
        //ถ้าไม่มี error
        ...
        http.post("products", data).then((response) => {
          ...
          // เมื่อเพิ่มสินค้าแล้ว แสดงล่าสุด
          this.getProducts(this.currentPage);
  ...
  validations() {
    return {
      name: {
        required: helpers.withMessage("ป้อนสินค้าก่อนจ้า", required),
      },
      slug: {
        required: helpers.withMessage("ป้อนสลักก่อนจ้า", required),
      },
      price: {
        required: helpers.withMessage("ป้อนราคาก่อนจ้า", required),
      },
    };
  },
  mounted() {

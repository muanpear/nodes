...
import http from "@/services/AuthService.js";
export default {
  data() {
    return {
      v$: useVuelidate(),
  ...
  validations() {
    return {
      email: {
        required: helpers.withMessage("ป้อนอีเมล์จ้า", required),
        email: helpers.withMessage("รูปแบบอีเมล์ไม่ถูก", email),
      },
      password: {
